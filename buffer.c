...
int buffer_push_data(socket_buffer* sb, char* msg, int sz)     //写数据到缓冲池中的新结点，然后sb指向新结点
{
    ...
    if (pool->head == NULL)  {                //缓冲池已空
        ...
        free_node = new_buffer_node(size); // 分配缓存池
        pool->len = size;
    }
    else  {
        free_node = pool->head;
    ...
    if (sb->head == NULL)  {
        sb->head = sb->tail = free_node;  // 头和尾都指向刚放入消息的节点
    }  else  {
...
void buffer_node_release(socket_buffer* sb)    //返回给缓冲池
{
    ...
    if (sb->head == NULL)  { 	// 释放完pool不含数据
        sb->tail = NULL;
    }
    free_node->next = pool->head;          //把该结点加入到缓冲池，头插法
...
char* buffer_read_spec(socket_buffer* sb, int sz, int* realSz)    //读取缓冲区sz个字节，实际读取数保存realSz里面
{
    if (sz == 0)  {
        return NULL;
    }
